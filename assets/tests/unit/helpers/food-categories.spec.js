import { mapEnabledCategories } from "@/helpers/category-categories";
...
const sessionWithoutAllocations = {
  id: 31,
  recipientId: 11,
  session: { id: 6, day: "Saturday", time: "11:00AM" },
  allocations: []
};
...
const sessionWithAllocations = {
  ...
  allocations: [
    ...
    { id: 74, categoryId: 5, quantity: "0.0", quantityLabel: "no boxes" }
  ]
};

describe("mapEnabledCategories", () => {
  it("marks category categories as enabled when session allocation exists", () => {
    const categories = mapEnabledCategories(sessionWithAllocations, categories);
    ...
    expect(actual.categoryId).toEqual(expected.categoryId);
  ...
  it("marks category categories as disabled when no session allocation exists", () => {
    const categories = mapEnabledCategories(sessionWithoutAllocations, categories);

    const expected = categories[0];
    const actual = categories.find(c => c.categoryId == expected.id);

    expect(categories.length).toEqual(categories.length);
    expect(actual.id).toEqual(null);
    expect(actual.categoryId).toEqual(expected.id);
